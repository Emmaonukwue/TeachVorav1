...
exports.getDashboard = async (req, res) => {
    // Show their pending and active jobs
    const [jobs] = await db.execute('SELECT * FROM jobs WHERE user_id = ?', [req.session.user.id]);
    res.render('pages/parent-dashboard', { user: req.session.user, jobs });
};
...
exports.submitRequest = async (req, res) => {
    ...
    try {
        // Insert as 'pending_review' (Admin must approve before publishing)
        await db.execute(
        ...
        res.redirect('/parent/dashboard?msg=Request+submitted+for+review');
    } catch (err) {
        console.error(err);
